 ...
 */
 ...
 void Heap<T>::remove(T item, void (*removeItemData)(T)) {
   // YOUR CODE IS HERE
   int removeInd = this-> getItem(item);
   if(removeInd == -1) return;
   elements[removeInd] = elements[count - 1];
   count -=1;
   reheapDown(removeInd);
   reheapUp(removeInd);
...
void Heap<T>:: heapsort(XArrayList<T>& arrayList){
  Heap<T> HeapToSort(this->comparator);
  XArrayList<T> Result;
  this-> clear();
  for (int i = 0; i < arrayList.size(); ++i) {
    HeapToSort.push(arrayList.get(i));
    cout<<HeapToSort.toString()<<endl;
}
for (int i= 0; i < arrayList.size(); i++){
  Result.add(i, HeapToSort.pop());  
}
arrayList = Result;
for(int i = 0; i < arrayList.size(); i++){
 ...
 void Heap<T>::reheapUp(int position) {
   ...
   while (position > 0) {
    ...
    if (aLTb(elements[parent], elements[position]) || compare(elements[parent], elements[position]) == 0) break; 
 ...
 void Heap<T>::reheapDown(int position) {
   ...
   int leftChild  = 2*position + 1;
    ...
    if (leftChild <= lastPos) {
        ...
        if (compare(elements[smallChild], elements[position]) < 0) {
