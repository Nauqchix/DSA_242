 ...
 */
 ...
 public:
     ...
     public:
         ...
         BWDIterator(DLinkedList<T> *pList = nullptr, bool begin = true)
         {
             this->pList = pList;
             if (begin) {
                 this->pNode = (pList != nullptr) ? pList->tail->prev : nullptr;
             } else {
         ...
         {
             Node* temp = pNode;
             pNode->prev->next = pList->tail;
             pList -> tail-> prev = pNode-> prev;
             pNode = pNode-> next;
             if (removeItemData){
                 removeItemData(pNode->data);
             }
 ...
 int DLinkedList<T>::indexOf(T item)
 {
     ...
     for(int i = 0; i < count; i++){
         if (equals(cursor-> data, item, itemEqual)) return i;
         cursor = cursor-> next;
     }
     return -1;
 ...
 bool DLinkedList<T>::removeItem(T item, void (*removeItemData)(T))
 {
     ...
     Node* cursor = head-> next;
     while(cursor != tail){
         if (equals(cursor->data, item, itemEqual)){
             cursor->prev->next = cursor->next;
             cursor->next->prev = cursor->prev;
             if (removeItemData) removeItemData(cursor->data);
             delete cursor;
             count--;
             return true;
         }
 ...
 void DLinkedList<T>::copyFrom(const DLinkedList<T> &list)
 {
     ...
     for (Node* cursor = list.head-> next; cursor != list.tail; cursor = cursor-> next){
         add(cursor->data);
     }
